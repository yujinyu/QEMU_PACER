...
#define BLK_MIG_COMPRESSION_LEVEL 9 
...
static int migr_progress; //add for prediction based on progress by Pacer

static int randomness = 0; //add by Pacer for large image size

typedef struct BlkMigDevState {
    BlockDriverState *bs;
    int bulk_completed;
    int shared_base;
    int scheduling; //add by Pacer for enabling scheduling
    int dirty_scheduling; //add by Pacer for enableing dirty scheduling 

    int64_t my_cur_sector[64]; // add by Pacer for randomness for large image
    int64_t current_segment; // above
    int64_t counter; // above
    int64_t my_segmentsize; // above
    int64_t my_countersize; // above
    int64_t my_fullcount;	


    WriteHistoryItem* currentHistoryItem; //add by Pacer   
    WriteFreqItem* currentFreqItem; //add by Pacer for 
    int access_done;
    int chunksize;
    int chunksize_bit;
...
typedef struct BlkMigState {
    ...
    int read_done;
    int transferred;
    uint64_t total_transferred; //add by Pacer for adaptive system
...
uint64_t my_blk_mig_bytes_transferred(void)
{
    ...
    // printf("my_blk_mig_bytes_transfer chunk %"PRId64" transferred %"PRId64"\n",block_mig_state.total_transferred,transferred);
     return transferred;
}
...
static inline long double compute_read_bwidth(void)
{
	assert(block_mig_state.total_time != 0);
        ...
        //printf("bulk read %"PRId64" , Bsize %d dirty read %"PRId64" dsize %d,traffic1 %"PRId64" traffic 2 %"PRId64" time %Le\n",block_mig_state.bulk_block_reads,BULK_BLOCK_SIZE,block_mig_state.dirty_block_reads,DIRTY_BLOCK_SIZE,traffic1,traffic2,block_mig_state.total_time);
...
//get the next chunk sector from freq list in bdrv
...
{  
	...
	if((block_mig_state.scheduling==1)&&(bmds->scheduling==1))
	{
		...
		blk->time = qemu_get_clock_ns(rt_clock);
                //modified from bdrv_aio_readv by Pacer
		...
		block_mig_state.submitted++;
                block_mig_state.total_transferred++; //add by Pacer for adaptive system
	...
	}else if(randomness==0)
	{
		...
		bmds->completed_sectors = cur_sector;
                 //Pacer: remove this line for transfering odd MB chunk, e.g. 9MB
	//	cur_sector &= ~((int64_t)BDRV_SECTORS_PER_BULK_CHUNK - 1);  
		...
		blk = qemu_malloc(sizeof(BlkMigBlock));
	//modify for LINUX native aio; must alignment buffer when O_DIRECT
		...
		blk->time = qemu_get_clock_ns(rt_clock);
                //modified from bdrv_aio_readv to my_bdrv_aio_readv by Pacer
		...
		int64_t total_sectors = bmds->total_sectors;
		int64_t i=bmds->current_segment;
		int64_t j=bmds->my_cur_sector[i];
		//printf("i %"PRId64" j %"PRId64"\n",i,j);	
		cur_sector = i*(bmds->my_segmentsize)+j;
		bmds->my_cur_sector[i]+=BDRV_SECTORS_PER_BULK_CHUNK;
		bmds->counter=bmds->counter+BDRV_SECTORS_PER_BULK_CHUNK;
		if(bmds->my_cur_sector[i]>=bmds->my_segmentsize)
			bmds->my_fullcount++;
		if(bmds->my_fullcount==64)
			return 1;
		if((bmds->counter)>=(bmds->my_countersize))
		{
		        i=rand()%64;
			while(bmds->my_cur_sector[i]>=bmds->my_segmentsize){
				i=rand()%64;
			}	
			bmds->current_segment=i;
			bmds->counter=0;
	...
	//modify for LINUX native aio; must alignment buffer when O_DIRECT
		...
		blk->nr_sectors=nr_sectors;
		blk->sector = cur_sector;
	//	printf("cur_sector %"PRId64" nr_sectors %d\n",cur_sector,nr_sectors);
		blk->dirty = false; 
		blk->iov.iov_base = blk->buf;
		blk->iov.iov_len = nr_sectors * BDRV_SECTOR_SIZE;
		qemu_iovec_init_external(&blk->qiov, &blk->iov, 1);
...
static void init_blk_migration_it(void *opaque, BlockDriverState *bs)
{
	...
	if (bs->type == BDRV_TYPE_HD) {
		...
		bmds->currentHistoryItem=NULL;
                ...
                //add by Pacer for large image randomness
	        int i=0;
		for(i=0;i<64;i++)
			bmds->my_cur_sector[i]=0;
		bmds->my_segmentsize=sectors / 64;
		bmds->current_segment=0;
		bmds->counter=0; 
		bmds->my_countersize=2048*32;                
		bmds->my_fullcount=0;
		printf("bmds my_segmentsize %"PRId64" countersize %"PRId64"\n",bmds->my_segmentsize,bmds->my_countersize);
...
void auto_set_write_block_distance(int64_t cur_sector, int nr_sectors, uint64_t count, uint64_t laccesstime, uint64_t distance, uint64_t variance)
{
	BlkMigDevState *bmds;
        QSIMPLEQ_FOREACH(bmds, &block_mig_state.bmds_list, entry) {
		set_write_block_distance(bmds->bs, cur_sector, nr_sectors, count, laccesstime, distance, variance);
        }
}
...
void set_dirtyblock_bitmap(int64_t cur_sector, int nr_sectors)
{
 	BlkMigDevState *bmds;
	QSIMPLEQ_FOREACH(bmds, &block_mig_state.bmds_list, entry) {
                bdrv_set_dirty(bmds->bs,cur_sector,nr_sectors);
 	        update_write_block_distance(bmds->bs,cur_sector,nr_sectors);
...
//get the next dirty sector from the dirty freq list
...
//add the dirty scheduling inside
...
//				blk->buf = qemu_malloc(DIRTY_BLOCK_SIZE); 
				...
				if (is_async) {
					...
					blk->time = qemu_get_clock_ns(rt_clock);
                                        //modified by Pacer from bdrv_aio_readv
					blk->aiocb = my_bdrv_aio_readv(bmds->bs, sector, &blk->qiov,
						nr_sectors, blk_mig_read_cb, blk);
					if (!blk->aiocb) {
						goto error;
...
//add by Pacer for adaptive system
/*
double get_unique_averate(void)
{
        BlkMigDevState *bmds;
        double averate = 0;

        QSIMPLEQ_FOREACH(bmds, &block_mig_state.bmds_list, entry) {
                averate += bdrv_get_dirty_unique_averate(bmds->bs);
        }
        return averate;
...
/*
int64_t get_unique_dirty(void)
{
        BlkMigDevState *bmds;
        int64_t dirty = 0;

        QSIMPLEQ_FOREACH(bmds, &block_mig_state.bmds_list, entry) {
                dirty += bdrv_get_dirty_unique_count(bmds->bs);
        }
...
static int is_stage2_completed(void)
{
	...
	if (block_mig_state.bulk_completed == 1) {
		...
		bwidth = compute_read_bwidth();
         //       printf("remain_dirty %"PRId64" bwidth %Le downtime %Le\n",remaining_dirty,bwidth,remaining_dirty/bwidth);
	//	printf("maxdowntime %ld,small? %d\n",migrate_max_downtime(),((remaining_dirty/bwidth)<=migrate_max_downtime()));
                if ((remaining_dirty / bwidth) <=
				migrate_max_downtime()) {
			/* finish stage2 because we think that we can finish remaing work
			   below max_downtime */
                        printf("--BELOW max_downtime--\n");
			return 1;
		}
	}
...
int64_t getNewExpireTime(int64_t speed,int64_t* pdirtyrate2,int64_t remain_time)
{
	 ...
	 BlkMigDevState *bmds;
        ...
        QSIMPLEQ_FOREACH(bmds, &block_mig_state.bmds_list, entry) {
                remaintime+=bdrv_getNewExpireTime(bmds->bs,pdirtyrate2,nextblock,speed,remain_time);
        }
        return remaintime;
...
/*
void set_all_clean_unique(void)
{
        BlkMigDevState *bmds;
        QSIMPLEQ_FOREACH(bmds, &block_mig_state.bmds_list, entry) {
                bdrv_set_clean_unique(bmds->bs,0,bmds->total_sectors);
                printCountMap(bmds->bs);
        }
...
static int block_save_live(Monitor *mon, QEMUFile *f, int stage, void *opaque)
{
	...
	if (stage == 1) {
		...
		//add for scheduling preparation
		...
		while ((block_mig_state.submitted +
			...
			if (block_mig_state.bulk_completed == 0) {
				...
				if (blk_mig_save_bulked_block(mon, f) == 0) {
					...
					//add for printing dirty block count
 				/*	time_t rawtime;
                                        struct tm * timeinfo;
                                        time ( &rawtime );
                                        timeinfo = localtime ( &rawtime );
...
/*		if(block_mig_state.throttling){
   	        	BlkMigDevState *bmds;
                 	QSIMPLEQ_FOREACH(bmds, &block_mig_state.bmds_list, entry) {
                    		printf("Zero delay Op in dirty iteration %"PRId64" total op is %"PRId64"\n",bmds->bs->zeroop,bmds->bs->totalopafterfcopy);
			}
		} */
		//end
		blk_mig_cleanup(mon);
...
static int block_load(QEMUFile *f, void *opaque, int version_id)
{
    ...
    do {
	    ...
	    bdrv_set_history_tracking(bs, 0);
            ...
            //add get length before receiving data, it is much flexible
...
static void block_set_params(int blk_enable, int shared_base, int sparse, int fs_bsize, int compression, int scheduling, int dscheduling, int throttling, void *opaque)
{
   ...
   /* shared base means that blk_enable = 1 */
    ...
    //add for sparse, compression and scheduling with throttling
